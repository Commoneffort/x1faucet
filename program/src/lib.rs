...
pub mod agent_faucet {
    ...
    pub fn claim_airdrop(ctx: Context<ClaimAirdrop>) -> Result<()> {
        ...
        // One-time only
        require!(!agent.has_claimed, FaucetError::AlreadyClaimed);

        let claim_amount = pool.claim_amount;

        // Tracked balance check
        require!(pool.balance >= claim_amount, FaucetError::InsufficientBalance);

        // Rent-exempt floor check — pool must keep enough lamports to stay alive
        let rent = Rent::get()?;
        ...
        // Transfer native XNT: faucet_pool PDA → agent wallet
        // faucet_pool is program-owned, so we manipulate lamports directly.
        ...
        **ctx.accounts.wallet.to_account_info().try_borrow_mut_lamports()?      += claim_amount;
        ...
        let revenue_share = claim_amount
            .checked_mul(pool.revenue_share_percent).unwrap()
        ...
        // Queue referral bonus (tracked, paid on repayment)
        if let Some(parent_key) = agent.parent {
            let referral_bonus = claim_amount
                .checked_mul(pool.referral_bonus_percent).unwrap()
                .checked_div(BASIS_POINTS).unwrap();
            treasury.total_referral_paid = treasury
                .total_referral_paid
    ...
    pub fn repay_debt(ctx: Context<RepayDebt>, amount: u64) -> Result<()> {
        ...
        require!(amount <= agent.debt, FaucetError::OverRepayment);

        // Agent wallet is system-owned signer → system_program::transfer
        let cpi_ctx = CpiContext::new(
            ctx.accounts.system_program.to_account_info(),
            SolTransfer {
    ...
    pub fn auto_repay(ctx: Context<AutoRepay>, earnings: u64) -> Result<()> {
        ...
        let repayment = earnings
            ...
            .checked_div(BASIS_POINTS).unwrap();
        let repayment = std::cmp::min(repayment, agent.debt);
    ...
    pub fn withdraw_treasury(ctx: Context<WithdrawTreasury>, amount: u64) -> Result<()> {
        ...
        let treasury = &mut ctx.accounts.treasury;
        require!(amount <= treasury.accumulated, FaucetError::InsufficientBalance);
        ...
        // Treasury PDA is program-owned → lamport manipulation
